...
class StepCEE(object):
    ...
    common_envelope_option_for_lambda : str
        ...
        1. 'default_lambda' : Use a constant value from the
          `common_envelope_lambda_default` parameter.

        2. 'lambda_from_grid_final_values' : Use the lambda parameter from
          MESA history, calculated using the same method as option 5 below.

        3. 'lambda_from_profile_gravitational' : Calculate lambda from the
        ...
        4. 'lambda_from_profile_gravitational_plus_internal' : As above,
    ...
    def CEE_simple_alpha_prescription(
        ...
        lambda1_CE,mc1_i,rc1_i = self.calculate_lambda_CE(core_definition_H_fraction,donor_prof,self.common_envelope_alpha_thermal)
        
        if double_CE:
            lambda2_CE,mc2_i,rc2_i = self.calculate_lambda_CE(core_definition_H_fraction,comp_prof,self.common_envelope_alpha_thermal)
        else:
        ...
        state = donor.state
        comp_state = comp_star.state
        
        # donor type
        if state in STAR_STATE_POST_MS:
            donor_type = 'He_core'
        elif state in STAR_STATE_POST_HeMS:
            donor_type = 'CO_core'
            core_element_fraction_definition = 0.1
        else:
            closest = difflib.get_close_matches(state, STAR_STATE_POST_MS + STAR_STATE_POST_HeMS)
            raise ValueError(f"type = {state!r} of donor not recognized. Closest match: {closest}")
        
        if double_CE:
            # companion type
            if comp_state in STAR_STATE_POST_MS:
                comp_type = 'He_core'
            elif comp_state in STAR_STATE_POST_HeMS:
                comp_type = 'CO_core'
                core_element_fraction_definition = 0.1
            else:
                closest = difflib.get_close_matches(comp_state, STAR_STATE_POST_MS + STAR_STATE_POST_HeMS)
